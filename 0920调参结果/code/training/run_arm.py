...
def main():
    ...
    parser.add_argument('--arm', required=True, choices=('hha', 'relplus'))
    ...
    try:
        ...
        def recorded_build(*positional, **keyword):
            ...
            actual_loss = {'type': type(criterion).__name__,
                           'focal_gamma': getattr(criterion, 'gamma', None),
                           'reduction': getattr(getattr(criterion, 'loss', None), 'reduction', None),
                           'ignore_index': getattr(getattr(criterion, 'loss', None), 'ignore_index', None)}
            if actual_loss != {'type': 'FocalLoss2d', 'focal_gamma': 1.0,
                               'reduction': 'none', 'ignore_index': 255}:
                raise RuntimeError('actual training criterion mismatch: {}'.format(actual_loss))
