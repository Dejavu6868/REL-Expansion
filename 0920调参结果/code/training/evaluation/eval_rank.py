...
def require_completed_training(suite, arm, output):
    root = Path(suite['output_root'])
    if (suite['arms'] != ['hha', 'relplus'] or suite['shared']['focal_gamma'] != 1 or
            root != Path('/data/zhuzhaoziao/RELPlus/outputs/CMX_S2D_B56_LR12_FG1_20260920/attempt2') or
            output != root / 'evaluation_epoch200' / arm):
        raise RuntimeError('worker output or gamma-1 dual-arm scope mismatch')
    preflight = json.loads((output.parent / 'preflight.json').read_text())
    queue = json.loads((root / 'train_status.json').read_text())
    if (preflight['status'] != 'PASS' or queue != preflight['training'] or queue['status'] != 'PASS' or
            queue['phase'] != 'train' or [row['arm'] for row in queue['completed']] != ['hha', 'relplus'] or
            queue['suite_sha256'] != digest(TRAIN_BUNDLE / 'suite.json') or
            preflight['bundle_manifest_sha256'] != digest(TRAIN_BUNDLE / 'bundle_manifest.json') or
            preflight['suite'] != suite):
        raise RuntimeError('worker requires unchanged successful formal-training and evaluation gates')
    checkpoint = root / 'runs' / arm / 'checkpoints' / 'epoch-200.pth'
    checked = preflight['arms'][arm]['checkpoint']
    current = checkpoint.stat()
    if (checked['path'] != str(checkpoint) or checked['size'] != current.st_size or
            checked['mtime_ns'] != str(current.st_mtime_ns)):
        raise RuntimeError('worker checkpoint differs from approved fixed epoch-200 endpoint')


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--arm', choices=('hha', 'relplus'), required=True)
    ...
    require_completed_training(suite, args.arm, args.output)
    if report.exists() or (args.output / 'evaluation' / ('rank_%02d_evaluation.json' % rank)).exists():
        raise RuntimeError('worker output already exists; refusing evaluation retry/overwrite')
    try:
        import cv2
        import torch
        cv2.setNumThreads(1)
