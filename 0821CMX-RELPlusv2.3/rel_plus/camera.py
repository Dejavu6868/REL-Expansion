"""Stanford2D3D perspective-camera geometry for REL+ v2.1."""
...
class CameraGeometry:
    ...
    def __post_init__(self):
        ...
        if not np.isclose(k_matrix[0, 1], 0.0, rtol=0.0, atol=1e-9) or not np.isclose(
            k_matrix[1, 0], 0.0, rtol=0.0, atol=1e-9
        ):
            raise ValueError("K_json must have zero skew")
        if not np.allclose(k_matrix[2], [0.0, 0.0, 1.0], atol=1e-9):
            raise ValueError("K_json must have final row [0, 0, 1]")
        cx_margin = 1.0
        cy_margin = 1.0
        if not (-cx_margin <= k_matrix[0, 2] <= intrinsics_shape[1] + cx_margin):
            raise ValueError("principal point cx is outside the image sanity range")
        if not (-cy_margin <= k_matrix[1, 2] <= intrinsics_shape[0] + cy_margin):
            raise ValueError("principal point cy is outside the image sanity range")
