...
def encode_rel_channels(
    ...
    points = np.asarray(points_for_encoding_cm)
    normals = np.asarray(normals_aligned)
    valid = np.asarray(valid_mask, dtype=bool)
    if points.ndim != 3 or points.shape[2] != 3:
        raise ValueError("points_for_encoding_cm must have shape HxWx3")
    if normals.shape != points.shape or valid.shape != points.shape[:2]:
        raise ValueError("normals_aligned and valid_mask must match points_for_encoding_cm")
    if tangent is None:
        ...
        if tangent_field.shape != points.shape:
            raise ValueError("tangent must match points_for_encoding_cm")
