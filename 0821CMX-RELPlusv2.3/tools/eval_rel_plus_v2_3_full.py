#!/usr/bin/env python3
"""Full-test CMX-REL+ V2.3 evaluator for one checkpoint."""
...
def load_checkpoint_once(network, checkpoint, *, expected_epoch):
    payload = torch.load(str(checkpoint), map_location="cpu")
    if not isinstance(payload, dict) or "epoch" not in payload:
        raise RuntimeError("formal checkpoint is missing payload epoch")
    payload_epoch = int(payload["epoch"])
    if payload_epoch != int(expected_epoch):
        raise RuntimeError(
            "checkpoint payload expected epoch {}, found {}".format(
                expected_epoch, payload_epoch
            )
        )
...
def main():
    ...
    checkpoint_epoch = load_checkpoint_once(
        ...
        per_class = [
            {
                "class_id": index,
                "class_name": name,
                "IoU_fraction": metrics["per_class_iou"][index],
                "IoU_percent": metrics["per_class_iou_percent"][index],
            }
