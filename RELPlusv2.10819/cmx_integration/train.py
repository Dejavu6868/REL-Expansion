...
with Engine(custom_parser=parser) as engine:
    ...
    seed = config.seed + (engine.local_rank if engine.distributed else 0)
    random.seed(seed)
    np.random.seed(seed)
    ...
    logger.info('Backbone: {}'.format(config.backbone))
    logger.info('Gate: {}'.format(config.using_gate))
    logger.info('SMMF: {}'.format(config.using_smmf))
    logger.info('DyMM: {}'.format(config.using_dymm))
    logger.info('Dataset: {}'.format(config.dataset_name))
    logger.info('Fold: {}'.format(config.dataset_fold))
