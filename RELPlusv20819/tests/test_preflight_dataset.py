...
def test_preflight_passes_matching_files_and_explicit_k_shape(tmp_path):
    result = PREFLIGHT.scan_row(write_case(tmp_path))
    assert result["status"] == "PASS"


def test_preflight_reports_k_mismatch_and_blocks_full_cache(tmp_path):
    result = PREFLIGHT.scan_row(write_case(tmp_path, declared_shape=(16, 16)))
    assert result["status"] == "FAIL"
    assert "K_RESOLUTION_MISMATCH" in result["reasons"]
...
def test_preflight_lists_gravity_singularity_before_batch_generation(tmp_path):
    ...
    result = PREFLIGHT.scan_row(write_case(tmp_path, rotation=rotation))
    assert result["status"] == "FAIL"
