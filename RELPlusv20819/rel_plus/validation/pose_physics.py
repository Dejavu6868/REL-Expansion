...
def _semantic_normal_metrics(labels, normals, semantic_ids, minimum_pixels):
    metrics = {}
    checks = []
    rules = {
        "floor": (-1.0, 0.8),
    ...
    for name, (target, tolerance) in rules.items():
        if name not in semantic_ids:
            continue
        mask = (labels == semantic_ids[name]) & np.all(np.isfinite(normals), axis=2)
...
def validate_pose_physics(
    ...
    *,
    ...
    """Return PASS/FAIL for strong evidence or NOT_APPLICABLE for weak-only data."""
    metrics = {}
    warnings = []
    strong_checks = []
    ...
    if labels is not None and normals_aligned is not None and semantic_ids is not None:
        ...
        semantic_metrics, semantic_checks = _semantic_normal_metrics(
            label_array, normal_array, semantic_ids, minimum_semantic_pixels
        )
        metrics.update(semantic_metrics)
        strong_checks.extend(semantic_checks)
        if not semantic_checks:
            warnings.append("semantic classes lacked enough valid normal pixels")

    if strong_checks:
        return PosePhysicsResult(
            "PASS" if all(strong_checks) else "FAIL",
            "strong",
            metrics,
            tuple(warnings),
        )
    ...
    return PosePhysicsResult("NOT_APPLICABLE", "weak", metrics, tuple(warnings))
