...
def generate_cache(rows, output_root, *, resume=False, workers=1):
    ...
    if workers == 1:
        outcomes = map(run, rows)
    else:
        executor = ThreadPoolExecutor(max_workers=workers)
        outcomes = executor.map(run, rows)
    try:
...
def main():
    ...
    selected = resolve_generation_scope(
        ...
        authorized=bool(config.full_cache_authorized),
    )
    ...
    if args.dry_run:
        ...
        _write_split_lists(generated, args.output_root)
    ...
    summary = {
        ...
        "resume": args.resume,
        "workers": args.workers,
        "full_cache_authorized": bool(config.full_cache_authorized),
        "full_cache_generated": bool(
            not args.dry_run and len(selected) == len(rows) and len(rows) > PILOT_LIMIT
        ),
