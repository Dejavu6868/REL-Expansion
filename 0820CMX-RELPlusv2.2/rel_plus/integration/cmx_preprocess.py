...
from utils.transforms import normalize as cmx_normalize
...
def sample_spatial_transform(input_shape, scales, output_shape, rng):
    ...
    is_python = rng is random or isinstance(rng, random.Random)
    is_numpy = isinstance(rng, np.random.Generator)
    if isinstance(rng, np.random.RandomState):
        raise TypeError("np.random.RandomState is not supported")
    if rng is np.random:
        raise TypeError("np.random module is not supported")
    if not (is_python or is_numpy):
        raise TypeError(
            "rng must be Python random.Random/random module or NumPy Generator"
        )
    source_height, source_width = (int(input_shape[0]), int(input_shape[1]))
    scale = float(rng.choice(tuple(float(value) for value in scales)))
    ...
    # Match the executable CMX crop sampler, including its inclusive upper
    # bound.  NumPy and Python expose different randint conventions.
    ...
    if is_numpy:
        ...
        crop_top = int(rng.randint(0, max_top)) if max_top else 0
        crop_left = int(rng.randint(0, max_left)) if max_left else 0
...
def _normalize(image, mean, std):
    return cmx_normalize(image, mean, std)
...
def analyze_invalid_interpolation(
    ...
    *,
    ...
    report = {
        ...
        "affected_pixel_count": int(np.count_nonzero(affected)),
    }

    if label is not None:
        label_array = np.asarray(label)
        if label_array.shape != valid_array.shape:
            raise ValueError("label shape must match valid_mask")
        if not np.issubdtype(label_array.dtype, np.integer):
            raise ValueError("label must be an integer array")
        if num_classes is None or int(num_classes) <= 0:
            raise ValueError("num_classes must be positive when label is supplied")
        transformed_label = _crop_and_center_pad(
            _resize(label_array, transform, cv2.INTER_NEAREST),
            transform,
            int(ignore_index),
        )
        affected_labels = transformed_label[affected]
        affected_count = int(affected_labels.size)
        denominator = max(1, affected_count)
        semantic = (
            (affected_labels >= 0) & (affected_labels < int(num_classes))
        )
        ignore_count = int(np.count_nonzero(affected_labels == ignore_index))
        semantic_count = int(np.count_nonzero(semantic))
        report.update(
            {
                "affected_label_ignore_count": ignore_count,
                "affected_label_valid_semantic_count": semantic_count,
                "affected_label_ignore_ratio": float(
                    ignore_count / denominator
                ),
                "affected_label_valid_semantic_ratio": float(
                    semantic_count / denominator
                ),
                "affected_label_class_counts": {
                    str(class_id): int(
                        np.count_nonzero(affected_labels == class_id)
                    )
