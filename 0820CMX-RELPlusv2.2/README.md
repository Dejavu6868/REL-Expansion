...
## CMX-REL+ V2.2 S2D integration in this copy

This independent copy upgrades the training, cache and scientific-evaluation
infrastructure around the frozen offline-480 REL+ v2.1 representation and
unchanged Original CMX model code. The X tensor remains three-channel
`[EGVIA, LOA, ReD]`; its valid mask is diagnostic only and is never passed to
the model. The formal configuration is fail-closed with
`training_authorized=False`, `full_cache_authorized=False` and
`data_ready=False`.

Read `CMX_RELPLUS_V2_2_SPEC.md` and `IMPLEMENTATION_REPORT_CN.md` first. The
1--3 sample entry is explicitly a smoke
(`tools/eval_rel_plus_v2_1_smoke.py`); the formal full-test entry is
`tools/eval_rel_plus_v2_2_full.py`. Runtime evidence lives outside the source
tree under
`/data/zhuzhaoziao/RELPlus/outputs/CMX_RELPlus_v2_2_integration`.
