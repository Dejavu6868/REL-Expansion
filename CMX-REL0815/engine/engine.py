...
class Engine(object):
    ...
    def restore_checkpoint(self):
        t_start = time.time()
        # Always load on CPU first to avoid a GPU RAM surge. Saved checkpoints
        # deliberately strip the DDP ``module.`` prefix, so only add it back
        # when restoring into a distributed wrapper.
        tmp = torch.load(self.continue_state_object, map_location=torch.device('cpu'))
        t_ioend = time.time()
        self.state.model = load_model(
            self.state.model, tmp['model'], is_restore=self.distributed)
