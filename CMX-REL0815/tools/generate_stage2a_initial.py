...
def main():
    ...
    selected = importlib.import_module("configs.stage2a_rawdepth")
    ...
    torch.save({"model": model.state_dict(), "seed": cfg.seed, "config": "configs.stage2a_rawdepth"}, temporary)
    os.replace(str(temporary), str(args.output))
    report = {
        "status": "PASS_COMMON_INITIAL_MODEL",
        "seed": cfg.seed,
        "source_config": "configs.stage2a_rawdepth",
