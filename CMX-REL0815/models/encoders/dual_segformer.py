...
def load_dualpath_model(model, model_file):
    ...
    if shape_mismatch:
        raise RuntimeError('MiT backbone shape mismatch: {}'.format(shape_mismatch))

    incompatible = model.load_state_dict(state_dict, strict=False)
    loaded_keys = sorted(key for key in state_dict if key in model_state)
    loaded_parameter_count = sum(int(model_state[key].numel()) for key in loaded_keys)
    model_parameter_count = sum(int(value.numel()) for value in model_state.values())
    ...
    report = {
        'checkpoint_path': os.path.abspath(model_file) if isinstance(model_file, str) else '<state_dict>',
        'checkpoint_sha256': checkpoint_hash,
        'loading_module': 'dual MiT backbone: identical weights copied to RGB and REL+ encoders',
        'loaded_tensor_count': len(loaded_keys),
        'loaded_parameter_count': loaded_parameter_count,
        'model_state_parameter_count': model_parameter_count,
        'loaded_parameter_ratio': loaded_parameter_count / float(model_parameter_count),
        'loaded_keys': loaded_keys,
        'missing_keys': sorted(incompatible.missing_keys),
        'unexpected_keys': sorted(incompatible.unexpected_keys),
        'checkpoint_unmapped_keys': sorted(raw_unmapped_keys),
        'shape_mismatch': shape_mismatch,
        'strict': False,
        'decoder_and_fusion_initialization': 'unchanged CMX defaults; decoder Kaiming, fusion module constructors',
    }
    report_path = os.environ.get('CMX_INITIALIZATION_REPORT')
    if report_path:
        os.makedirs(os.path.dirname(os.path.abspath(report_path)), exist_ok=True)
        temporary = '{}.{}.tmp'.format(report_path, os.getpid())
        with open(temporary, 'w') as handle:
            json.dump(report, handle, indent=2, sort_keys=True)
            handle.write('\n')
        os.replace(temporary, report_path)
    logger.info(
        'Pretrained initialization: loaded %d tensors / %d parameters; '
        'missing=%d unexpected=%d unmapped=%d shape_mismatch=%d',
        len(loaded_keys), loaded_parameter_count, len(incompatible.missing_keys),
        len(incompatible.unexpected_keys), len(raw_unmapped_keys), len(shape_mismatch))
    del state_dict
