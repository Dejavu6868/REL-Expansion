...
def save_review_bundle(output_dir, rgb_bgr, debug, rgb_label="RGB"):
    """Save the nine required review views and one labelled montage."""
    destination = Path(output_dir)
    destination.mkdir(parents=True, exist_ok=True)
    valid = np.asarray(debug["valid_mask"], dtype=bool)
    ...
    rel_display = cv2.cvtColor(rel_plus, cv2.COLOR_RGB2BGR)
    ...
    files = {
        ...
        "loa.png": rel_plus[:, :, 1],
        "rel_plus_display.png": rel_display,
    }
    ...
    panels = [
        ...
        ("display RGB := EGVIA/LOA/ReD", rel_display),
    ]
