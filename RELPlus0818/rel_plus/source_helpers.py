...
``SOURCE_AUDIT.md`` and ``REL_TO_REL_PLUS_MAPPING.md``.
"""
...
def estimate_source_perspective_normals(depth_m, valid_mask, k_json, radius=2):
    """Estimate source-aligned normals from metric perspective z-depth."""
    if radius != 2:
    ...
    normal_valid = np.asarray(valid_mask, dtype=bool) & np.all(
        np.isfinite(normals), axis=2
    )
...
def align_points_and_normals_to_gravity(points, normals, gravity_camera):
    """Apply the original REL getRMatrix(...), then rotatePC(..., R.T)."""
    ...
    source_rotation = get_r_matrix(world_down.T, gravity)
    alignment = source_rotation.T
    aligned_points = rotate_pc(np.asarray(points), alignment)
    aligned_normals = rotate_pc(np.asarray(normals), alignment)
    return aligned_points, aligned_normals, alignment
