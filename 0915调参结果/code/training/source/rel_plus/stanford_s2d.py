...
def load_canonical_frame(depth_path, camera_json_path, *, dataset_profile):
    """Load a profile-bound native Depth16/pose and canonicalize it once."""
    ...
    source_shape = tuple(raw_depth.shape)
    dataset_profile.assert_native_image_shape(source_shape)
    camera = load_stanford_s2d_camera_geometry(
        camera_json_path, dataset_profile.native_image_shape
    )
    dataset_profile.assert_camera_reference(camera)
    canonical_shape = dataset_profile.canonical_image_shape
    if source_shape != canonical_shape:
        raw_depth = resize_raw_depth_nearest(raw_depth, canonical_shape)
        camera = resize_camera_geometry(camera, canonical_shape)
