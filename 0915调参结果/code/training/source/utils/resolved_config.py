...
def apply_resolved_config(config, path):
    ...
    if config.integration_protocol_id == "CMX_RELPLUS_V2_3":
        if overrides.get("source_compatible_invalid_accepted") is not True:
            raise RuntimeError(
                "resolved config must explicitly accept SOURCE_COMPAT_STORAGE_255"
            )
    elif config.integration_protocol_id == "CMX_S2D_THREE_ARM_V1":
        if overrides.get("source_compatible_invalid_accepted") is not False:
            raise RuntimeError(
                "three-arm resolved config must not claim REL+ invalid acceptance"
            )
