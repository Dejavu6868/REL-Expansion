...
THREE_ARM_INTEGRATION_PROTOCOL_ID = "CMX_S2D_THREE_ARM_V1"
...
def assert_three_arm_data_ready(config):
    report = _load_json_report(config.input_audit_report, "three-arm input audit")
    if report.get("status") != "PASS":
        raise RuntimeError("three-arm input audit status must be PASS")
    if report.get("blocks_future_three_arm_comparison") is not False:
        raise RuntimeError("three-arm input audit still blocks the comparison")
    if _absolute(report.get("manifest_path", "")) != _absolute(
        config.full_manifest
    ):
        raise RuntimeError("three-arm input audit manifest mismatch")

    arm = getattr(config, "comparison_arm", None)
    if arm == "rgbd":
        section = report.get("raw_depth", {})
        expected_status = "RGBD_INPUT_READY"
        expected_root = report.get("raw_depth_root")
        expected_shape = {"480x480": 70496}
    elif arm == "hha":
        section = report.get("hha", {})
        expected_status = "HHA_INPUT_READY"
        expected_root = report.get("hha_root")
        expected_shape = {"480x480x3": 70496}
    else:
        raise RuntimeError("unknown three-arm comparison arm")
    if _absolute(expected_root or "") != _absolute(config.x_root_folder):
        raise RuntimeError("three-arm input audit X root mismatch")
    if section.get("status") != expected_status:
        raise RuntimeError("three-arm input audit arm status mismatch")
    if section.get("file_count") != 70496 or section.get("failure_count") != 0:
        raise RuntimeError("three-arm input audit file coverage mismatch")
    if section.get("dtype_counts") != {"uint8": 70496}:
        raise RuntimeError("three-arm input audit dtype mismatch")
    if section.get("shape_counts") != expected_shape:
        raise RuntimeError("three-arm input audit shape mismatch")

    preflight = _load_json_report(
        config.training_data_preflight_report, "REL+ RGB/label preflight"
    )
    if preflight.get("status") != "PASS":
        raise RuntimeError("shared RGB/label preflight status must be PASS")
    if preflight.get("sample_count") != 70496:
        raise RuntimeError("shared RGB/label preflight sample count mismatch")
    if preflight.get("all_samples_decoded_this_run") is not True:
        raise RuntimeError("shared RGB/label preflight must cover all samples")
    if _absolute(preflight.get("class_mapping", "")) != _absolute(
        config.class_mapping
    ):
        raise RuntimeError("shared RGB/label class mapping mismatch")
    split_counts = validate_ordered_runtime_splits(config)
    return {
        "status": "PASS",
        "input_audit": report,
        "shared_rgb_label_preflight": preflight,
        "split_counts": split_counts,
    }
...
def assert_training_ready(config):
    ...
    if (
        getattr(config, "integration_protocol_id", None)
        == THREE_ARM_INTEGRATION_PROTOCOL_ID
    ):
        return assert_three_arm_data_ready(config)
...
def assert_runtime_dataset_contract(config, *, require_cache_audit=False):
    ...
    if require_cache_audit:
        if getattr(config, "integration_protocol_id", None) == V2_3_INTEGRATION_PROTOCOL_ID:
            ...
            reports = assert_three_arm_data_ready(config)
            counts.update(reports)
        else:
