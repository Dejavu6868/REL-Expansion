...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for epoch in range(engine.state.epoch, config.nepochs + 1):
            ...
            recovery_step = int(
                getattr(config, "recovery_checkpoint_step", 0) or 0
            )
            recovery_before = int(
                getattr(
                    config,
                    "recovery_checkpoint_before_epoch",
                    config.checkpoint_start_epoch,
                )
            )
            recovery_due = (
                recovery_step > 0
                and epoch < recovery_before
                and epoch % recovery_step == 0
            )
            if checkpoint_due or recovery_due:
