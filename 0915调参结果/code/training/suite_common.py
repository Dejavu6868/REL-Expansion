...
EXPECTED_SHARED = {
    ...
    'focal_gamma': 2, 'loss_reduction': 'none_then_mean',
...
def validate_paths(suite):
    ...
    if source.parent != Path('/home/zhuzhaoziao/RELPlus') or not source.name.startswith('CMX-S2D-B56-LR12-'):
        raise ValueError('independent source directory required')
    if (output.parent.parent != Path('/data/zhuzhaoziao/RELPlus/outputs') or
            not output.parent.name.startswith('CMX_S2D_B56_LR12_') or
...
def load_suite(path):
    ...
    if suite.get('arms') != list(ARMS) or suite.get('authorization', {}).get('approved') is not True:
        raise ValueError('exact three-arm scope and explicit training authorization required')
    return suite
