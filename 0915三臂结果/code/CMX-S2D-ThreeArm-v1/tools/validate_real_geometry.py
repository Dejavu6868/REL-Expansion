...
def validate_row(row, semantic_ids):
    ...
    _, debug = generate_rel_plus_v2_1(
        canonical_raw, canonical_camera, return_debug=True
    )
    ...
    if xyz_path.is_file():
        ...
        pose_result = validate_pose_physics(
            ...
            semantic_valid_mask=(
                debug["depth_valid"]
                & debug["normal_finite_mask"]
                & debug["normal_nonzero_mask"]
                & debug["normal_quality_mask"]
            ),
    ...
    return {
        ...
        "sample_status": (
            "PASS"
            if geometry_status in ("PASS", "NOT_APPLICABLE")
            and pose_result.status in ("PASS_STRONG", "PASS_WEAK")
            else "FAIL"
        ),
...
def main():
    ...
    summary = {
        ...
        "pose_status_counts": {
            ...
            for status in (
                "PASS_STRONG", "PASS_WEAK", "REVIEW_REQUIRED", "FAIL", "NOT_APPLICABLE"
            )
        },
