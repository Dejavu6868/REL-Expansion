...
def test_sweep_does_not_pass_legacy_use_env_to_modern_launcher(tmp_path):
    from tools.eval_checkpoint_sweep_v2_3 import build_evaluator_command
...
def test_real_ddp_smoke_contains_step_save_restore_and_parameter_groups():
    ...
    for evidence in (
        ...
        "rank_runtime_state_restored",
        "sampler_epoch_restored",
    ):
        assert evidence in source
    assert "DISPOSABLE_DDP_SMOKE" in source


def test_second_checkpoint_allows_existing_log_symlink(tmp_path):
    from engine.engine import Engine

    log_dir = tmp_path / "formal_run" / "logs"
    log_dir.mkdir(parents=True)
    log_dir_link = tmp_path / "latest_formal_logs"
    checkpoint_dir = tmp_path / "checkpoints"
    engine = Engine.__new__(Engine)
    engine.state = SimpleNamespace(epoch=100)

    def save_checkpoint(path):
        Path(path).write_text(
            "epoch={}\n".format(engine.state.epoch), encoding="utf-8"
        )

    engine.save_checkpoint = save_checkpoint
    engine.save_and_link_checkpoint(
        str(checkpoint_dir), str(log_dir), str(log_dir_link)
    )
    assert log_dir_link.is_symlink()
    assert log_dir_link.resolve() == log_dir.resolve()

    engine.state.epoch = 105
    engine.save_and_link_checkpoint(
        str(checkpoint_dir), str(log_dir), str(log_dir_link)
    )
    assert (checkpoint_dir / "epoch-105.pth").read_text(
        encoding="utf-8"
    ) == "epoch=105\n"
    assert (checkpoint_dir / "epoch-last.pth").resolve() == (
        checkpoint_dir / "epoch-105.pth"
    ).resolve()


def test_formal_resume_checkpoint_and_command_are_fail_closed(tmp_path):
    from tools.launch_formal_training_v2_3 import (
    ...
    checkpoint = checkpoint_dir / "epoch-100.pth"
    torch.save(
        {
    ...
    config = SimpleNamespace(
        ...
        checkpoint_epochs=list(range(100, 201, 5)),
        niters_per_epoch=6613,
        nepochs=200,
    )
    identity = validate_resume_checkpoint(checkpoint, config)
    assert identity == {
        "path": str(checkpoint),
        "epoch": 100,
        "iteration": 6612,
    ...
    assert command[-2:] == ["--continue", str(checkpoint)]
    ...
    with pytest.raises(RuntimeError, match="filename/payload epoch mismatch"):
