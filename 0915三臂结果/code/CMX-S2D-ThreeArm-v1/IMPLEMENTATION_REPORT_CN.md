...
2026-08-25 更新：attempt1 在完成 epoch 105 后、保存第二个 checkpoint 前因
合法日志软链接被误判为自引用而退出 1。epoch-100 模型与 optimizer payload
完整可读。该缺陷已按 RED/GREEN 修复，完整 live-source 回归 126 passed，
attempt2 已从 epoch 100 恢复并确认超过 100 次新 optimizer update。详见
`FORMAL_TRAINING_RECOVERY_REPORT.md`。200 epochs 仍未完成。
...
### Attempt1 初始启动（历史证据，已由 attempt2 恢复替代）
