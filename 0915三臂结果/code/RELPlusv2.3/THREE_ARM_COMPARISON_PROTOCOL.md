# RGBD / HHA / REL+ future comparison protocol

V2.3 authorizes only the REL+ single arm. Before any future three-arm launch,
the read-only RawDepth/HHA result in `THREE_ARM_DATA_CONTRACT.md` must also be
resolved. A RawDepth or HHA contract issue does not block the current REL+ arm.
