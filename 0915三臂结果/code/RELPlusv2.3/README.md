...
## CMX-REL+ V2.3 S2D formal-cache and training infrastructure

This independent V2.3 copy closes the full-cache, full-data preflight, real
8-rank optimizer/checkpoint smoke, formal-launch and evaluation interfaces
around the frozen offline-480 REL+ v2.1 representation. The X tensor remains
three-channel `[EGVIA, LOA, ReD]`; its valid mask remains diagnostic only and
is never passed to Original CMX. Repository defaults remain fail-closed with
`training_authorized=False` and `full_cache_authorized=False`.

Runtime status on 2026-08-25: `FORMAL_TRAINING_STARTED`. Attempt1 stopped after
epoch 105 because a valid existing log symlink was misclassified during the
second formal checkpoint. The defect has a RED/GREEN regression guard;
attempt2 passed all resume gates and is running from the complete epoch-100
model/optimizer checkpoint. See `FORMAL_TRAINING_RECOVERY_REPORT.md` for the
failure and recovery boundary. The 200 epochs and scientific evaluation are
not yet complete.

Read `CMX_RELPLUS_V2_3_SPEC.md`, `FORMAL_TRAINING_PROTOCOL.md` and
`IMPLEMENTATION_REPORT_CN.md` first. Runtime evidence is intentionally kept
...
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/cmx-cross-modal-fusion-for-rgb-x-semantic/semantic-segmentation-on-bjroad)](https://paperswithcode.com/sota/semantic-segmentation-on-bjroad?p=cmx-cross-modal-fusion-for-rgb-x-semantic)
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/cmx-cross-modal-fusion-for-rgb-x-semantic/semantic-segmentation-on-tlcgis)](https://paperswithcode.com/sota/semantic-segmentation-on-tlcgis?p=cmx-cross-modal-fusion-for-rgb-x-semantic)
[![PWC](https://img.shields.io/endpoint.svg?url=https://paperswithcode.com/badge/cmx-cross-modal-fusion-for-rgb-x-semantic/semantic-segmentation-on-porto)](https://paperswithcode.com/sota/semantic-segmentation-on-porto?p=cmx-cross-modal-fusion-for-rgb-x-semantic)
