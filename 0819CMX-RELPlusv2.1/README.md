...
## CMX-REL+ v2.1 S2D integration in this copy

This independent copy integrates the frozen offline-480 REL+ v2.1
representation into unchanged Original CMX model code. The X tensor remains
three-channel `[EGVIA, LOA, ReD]`; its valid mask is diagnostic only and is
never passed to the model. The formal configuration is intentionally blocked
with `training_authorized=False` and `data_ready=False`.

Read `CMX_RELPLUS_V2_1_INTEGRATION_SPEC.md` first. The only canonical REL+
evaluation entry is `tools/eval_rel_plus_v2_1.py`; legacy evaluation entries
explicitly reject REL+ mode. Runtime validation artifacts live outside the
source tree under
`/data/zhuzhaoziao/RELPlus/outputs/CMX_RELPlus_v2_1_integration_fix`.
